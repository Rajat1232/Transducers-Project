...
class IrrigationSystem:
    ...
    def setup_plot(self):
        ...
        self.line_temp, = ax.plot([], [], 'r-', label='Temperature')
    ...
    def update_plot(self, frame):
        ...
        try:
            if self.ser and self.ser.is_open and self.ser.in_waiting:
                ...
                if len(data) == 6:
                    timestamp, temp, humidity, moisture, voltage, pump = map(float, data)
                    print(f"Processed: Time={timestamp}, Temp={temp}, Humidity={humidity}, Moisture={moisture}, Voltage={voltage}, Pump={pump}")

                    self.time_data.append(timestamp / 1000)
                    ...
                    # Update crop suggestions based on new sensor data
                    self.suggest_crops()
                else:
                    print(f"Invalid data format: {data}")
            else:
                print("No data available or serial port not open")
        except Exception as e:
